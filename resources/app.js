...
function generateLibrary(bookCollection) {
    ...
    for(let i = 0; i < bookCollection.length; i++) {
        ...
        hasReadBtn.classList.add("toggleRead");
...
// Add base database
const favBook = new Book("Think Rich, Grow Rich", "Napoleon Hill", "self-help", true);
addBookToLibrary(favBook);
const moneyBook = new Book("Richest Man In Babylon", "Hill", "personal finance", true);
...
submitNewBook.addEventListener("submit", (e) => {
    e.preventDefault();
    const newBook = new Book(e.target.title.value, e.target.author.value, e.target.genre.value, e.target.read.checked);
...
const status = document.getElementById("printLibrary");
